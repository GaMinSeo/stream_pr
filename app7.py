...
# 현재 시간을 가져와서 유니크한 파일명 만드는데 사용하기 위함
...
def main():
    ...
    if choice == menu[0] :
        ...
        file = st.file_uploader('이미지 파일을 선택하세요.',type=['jpg','png','jpeg'])
        ...
        if file is not None :
            ...
            # 현재 시간을 유저의 아이디를 조합해서 만드는게 실무에서 가장 많이 사용
            current_time = datetime.now()
            # 디버깅용 print
            print( current_time.isoformat().replace(':','_') + '.jpg') # 문자열로 바꾸는 함수. isoformat
        ...
        file = st.file_uploader('CSV 파일을 선택하세요', type='csv')
