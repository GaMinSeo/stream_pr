...
def main():
    ...
    status = st.radio('정렬 방법 선택하세요',my_order)

    print(status)

    # petal_length 컬럼을 정렬해서 df를 보여준다.
    if status == my_order[0] :
        st.dataframe(df.sort_values('petal_length' , ascending=True))
    if status == my_order[1] :
        st.dataframe(df.sort_values('petal_length' , ascending=False))
    
    # 체크박스 : 둘 중에 하나만 선택하게끔 만들때. (체크 / 해제)
    ...
    if my_choice == language[0] or my_choice == language[2]:
        ...
        st.text('배우고 싶은 언어입니다.')
    else:
        st.text('오래된 언어입니다.')
    
    # 멀티셀렉트 : 여러개 중에서, 여러개를 선택하게 할때.

    # 유저가 선택한 컬럼을, 데이터 프레임으로 보여주되
    # 아무것도 선택안하며느 아무것도 나오지 않게 하시오
    ...
    # 나이를 슬라이더로 입력받는다.
    # 1세부터 120세까지 입력 받을 수 있도록 한다
    # 선택한 나이가 웹화면에 출력되도록 한다. '선택한 나이는 33세 입니다.'
    age = st.slider('나이 선택', 0, 120, 0, 1)
    age
